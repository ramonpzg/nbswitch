...
class NbSwitch:
    ...
    def cells_token_counter(self):
        ...
        if self.cells_token_count < 3_800:
            ...
            answer = input (f"Your notebook is quite large with roughly {self.cells_token_count} tokens that need to be changed, are you sure you want to continue? (y/n) ")
            if answer == 'y':
                self.passes = 5
            elif answer == 'n':
                exit()
    ...
    def save_doc(self, output_file = None):
        if output_file:
